//Implement the three basic smart pointer interfaces and pointer traits
...
namespace my_stl {
    ...
    class __ref_counter;
    template <typename _Tp, typename _Dp, typename Alloc> class __ctrl_blk;
    ...
    //One notable thing is that for deleting is determined at runtime (virtual call)
    //for unique_ptr, the following would not work:
    //class Foo;  //forward declaration
    //unique_ptr<FOO> make_foo(); // forward declaration
    //unique_ptr<Foo> foo_ptr= make_foo();
    //...foo_ptr go out of scope
    //the reason is that unique_ptr do not have any overhead, the delete function of FOO
    //is staticly binded, when foo_ptr go out of scope, it will certainly call delete, 
    //which must know the size of FOO.
    //
    //Shared_ptr poses a different approach, since overhead has been introduced by having
    //control block initialization and being shared by owners, it make sense to overcome
    //this problem by introduce overhead. The way to do this is to have a abstract class with
    //deleting interface, and a impl class deal with the actual implementation
    //shared_ptr only holds pointer to the abstract class. So when compiler need to generate code
    //for shared_ptr<FOO> 's dtor, it does not need to know delete expression is being called
    //the actually deletion is being addressed at run-time
    //------------------------------reference counter-----------------------------------
    class __ref_counter {
        private:
    ...
    constexpr __ref_counter::__ref_counter(long refs):
        __shared_owners(refs),
        __weak_owners(refs) {}

    __ref_counter::~__ref_counter() {}

    void __ref_counter::add_shared() noexcept {
        ++__shared_owners;
    }

    void __ref_counter::add_weak() noexcept {
        ++__weak_owners;
    }

    void __ref_counter::release_shared() noexcept {
        --__shared_owners;
    }

    void __ref_counter::release_weak() noexcept {
        --__weak_owners;
    }

    long __ref_counter::shared_count() const noexcept {
        return __shared_owners;
    }

    long __ref_counter::weak_count() const noexcept {
        return __weak_owners;
    }
    //--------------------------end of reference counter---------------------------------
