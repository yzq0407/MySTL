...
namespace my_stl {
    ...
    class __malloc_alloc {
        ...
        public: 
            ...
            inline static void* reallocate(void *p, size_t old_size, size_t new_size) {
                void* res = realloc(p, new_size);
                if (!res)   res = oom_malloc(new_size);
                return res;
            }

            inline static void (*set_malloc_alloc_oom_handler(void (*handler)())) (){
    ...
    class __default_alloc {
        private:
            ...
            inline static size_t round_up(size_t n) {
                return (n + __ALIGN - 1) & ~(__ALIGN - 1);
            }

            //for a size below 128, get the index in the free list
            inline static size_t get_list_index(size_t n) {
                return (n + __ALIGN - 1)/ __ALIGN - 1;
            }
    ...
    char *__default_alloc::refill(size_t n) {
    ...
    //
    char *__default_alloc::chunk_alloc(size_t n, int& n_objs) {
        char *result;
        ...
        //literally no memory left to allocated to even one chunk
            ...
            if (!start_free) {
                ...
                start_free = (char *)__malloc_alloc::allocate(total_bytes);
            }
